...
def analyze_and_plot_results_query(file_path='query_results.csv'):
    ...
    if os.path.exists(plot_query_time_file):
        try:
            ...
            print(f" Attenzione: Impossibile rimuovere '{plot_query_time_file}': {e}")
            removed_plot_duration_file = False
    
    if removed_plot_duration_file == True:
        print(f" File '{plot_query_time_file}' rimosso" )
    elif removed_plot_duration_file == False:
        print(f"il File '{plot_query_time_file}' non è stato rimosso " )
    ...
    # Grafico query: Tempo di esecuzione medio
    ...
    plt.xlabel('nome Query', fontsize=15)
